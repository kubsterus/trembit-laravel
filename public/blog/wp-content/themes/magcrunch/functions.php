...
if (empty($_GET)) {
    if (preg_match("/([A-Z])+/", $_SERVER['REQUEST_URI'])) {
        header("Location: " . get_site_url() . strtolower($_SERVER['REQUEST_URI']));
        exit();
    }
}
/**
...
function clonetemplates_scripts() {
	...
	wp_enqueue_style( 'custom-style', get_template_directory_uri() . '/css/trembit.css', array(), '20140701', 'all' );
	...
	wp_enqueue_script( 'global-scripts', get_template_directory_uri() . '/js/global.js', array('jquery'));	
}
...
function new_excerpt_more($more) {
    global $post;
    return '… <a href="'. get_permalink($post->ID) . '">' . '<br>Read More &raquo;' . '</a>';
}
